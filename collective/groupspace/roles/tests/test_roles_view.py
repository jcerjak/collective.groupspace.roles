...
class TestRolesView(PloneTestCase):
    ...
    def afterSetUp(self):
        ...
        self.portal.acl_users._doAddUser(
            'nonasciiuser', 'secret', ['Member'], [])
        nonasciiuser = self.portal.portal_membership.getMemberById(
            'nonasciiuser')
    ...
    def test_search_by_login_name(self):
        ...
        self.failUnless(
            len(results) and results[0].get('id') == 'testuser',
            msg="Didn't find testuser when I searched by login name."
        )

    def test_search_with_nonascii_users(self):
        """Make sure we can search with users that have non-ascii-chars
        in their fullname.
        """
        request = self.app.REQUEST
        ...
        self.failUnless(
            len(results) and
            results[0].get('title') == '\xc3\x84\xc3\x9c\xc3\x9f',
            msg="Umlaute"
        )
